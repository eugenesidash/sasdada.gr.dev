...
$db->query("UPDATE LOW_PRIORITY `" . PREFIX . "_users` SET user_invisibility = '0', invisibility_date = '0' WHERE user_id = '{$user_info['user_id']}'");
    ...
    //Если у пользователя закончился срок действия пакета подарков то убираем все
	$giftfree = $db->super_query("SELECT giftfree_count, giftfree_date FROM `".PREFIX."_users` WHERE user_id = '{$user_info['user_id']}'");
    if($giftfree['giftfree_date'] < $server_time){
	$db->query("UPDATE LOW_PRIORITY `".PREFIX."_users` SET giftfree_active = '0', giftfree_count = '0', giftfree_date = '0' WHERE user_id = '{$user_info['user_id']}'");
	}elseif($giftfree['giftfree_count'] == 0){
	$db->query("UPDATE LOW_PRIORITY `".PREFIX."_users` SET giftfree_active = '0', giftfree_count = '0', giftfree_date = '0' WHERE user_id = '{$user_info['user_id']}'");
	}
