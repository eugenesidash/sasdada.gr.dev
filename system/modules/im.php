...
HTML;
		...
		case "upDialogs":
			...
			if($update){
				...
				if($user_info['user_pm_num']){
					$user_pm_num_2 = "<div class=\"headm_newac\" style=\"margin-left:14px\">{$user_info['user_pm_num']}</div>";
		...
		default:
			...
			//Вывод диалогов
			$sql_ = $db->super_query("SELECT tb1.msg_num, user_last_visit, im_user_id, tb2.user_search_pref, user_photo FROM `".PREFIX."_im` tb1, `".PREFIX."_users` tb2 WHERE tb1.iuser_id = '".$user_id."' AND tb1.im_user_id = tb2.user_id ORDER by `idate` DESC LIMIT 0, 50", 1);
