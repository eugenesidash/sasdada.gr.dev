...
if ($discussion_type == 1)
            $sql_order_discussion = "`fdate` DESC";
    elseif ($discussion_type == 2)
            $sql_order_discussion = "`lastdate` DESC";
    elseif ($discussion_type == 3)
            $sql_order_discussion = "`msg_num` DESC";
    else
            $sql_order_discussion = "`lastdate` DESC";
...
if ($discussion_type != 4)
    $sql_where_discussion_cnt .= "WHERE title LIKE '%{$query}%'";
    else
    $sql_where_discussion_cnt .= "AND title LIKE '%{$query}%'";
}
	...
	if($sql_){
		...
		//Если критерий поиск "по аудизаписям"
            ...
            foreach ($sql_ as $row) {
                ...
                if ($row['photo'])
                    $tpl->set('{ava}', '/uploads/groups/' . $row['public_id'] . '/50_' . $row['photo']);
                else
                    $tpl->set('{ava}', '{theme}/images/no_ava_50.png');
