...
if($logged){
	...
	switch($act){
		...
		case "delpublic":
			...
			$row = $db->super_query("SELECT real_admin FROM `".PREFIX."_communities` WHERE id = '{$id}'");
			if($row['real_admin'] == $user_id){
			
				$db->query("UPDATE `".PREFIX."_communities` SET del = '1',data_del = '".$server_time."' WHERE id = '{$id}'");
		...
		case "overspublic":
			...
			$row = $db->super_query("SELECT real_admin,data_del,del FROM `".PREFIX."_communities` WHERE id = '{$id}'");
			if($row['real_admin'] == $user_id){
				...
				if($row['del']!=0) {
					...
					if(($server_time-$row['data_del'])<86400) {
						...
						$db->query("UPDATE `".PREFIX."_communities` SET del = '0',data_del = '0' WHERE id = '{$id}'");
		...
		case "saveinfo":
			...
			$_POST['onv'] = str_replace(array('"', "'"), '', $_POST['onv']);
            $onv = ajax_utf8(textFilter($_POST['onv'], false, true));
			...
			if(stripos($checkAdmin['admin'], "u{$user_id}|") !== false AND isset($title) AND !empty($title) AND $adress_ok){
				...
				if(!$checkAdres['cnt'] OR $adres_page == ''){
					$db->query("UPDATE `".PREFIX."_communities` SET title = '".$title."',onv = '".$onv."', descr = '".$descr."', lastnews = '".$lastnews."',comments = '".$comments."',pages = '{$pages}', discussion = '{$discussion}',onv = '{$onv}', adres = '".$adres_page."', web = '{$web}' WHERE id = '".$id."'");
					if(!$adres_page)
						echo 'no_new';
				} else
					echo 'err_adres';
