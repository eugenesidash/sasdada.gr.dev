...
if($logged){
	...
	$user_id = $user_info['user_id'];
		...
		case "save_descr":
			...
			$system_albums = $db->super_query("SELECT aid FROM `".PREFIX."_communities_albums` WHERE  pid = '{$pid}'");
			if($row and $row['album_id'] != $system_albums['aid']){
		...
		case "editphoto":
			...
			$row = $db->super_query("SELECT descr,  album_id FROM `".PREFIX."_communities_photos` WHERE id = '{$id}' AND pid = '{$pid}'");
			$system_albums = $db->super_query("SELECT aid FROM `".PREFIX."_communities_albums` WHERE system = '0' and pid = '{$pid}'");
			if($row and $row['album_id'] != $system_albums['aid'])
		...
		default:
			...
			//Выводим данные о владельце альбома(ов)
			$row_owner = $db->super_query("SELECT title, id, albums_num, new_mark_photos, adres, admin FROM `".PREFIX."_communities` WHERE id = '{$uid}'");
						...
						if($row_owner['albums_num']){
							if(!$_POST['page_cnt']) {
								...
								if(!$_POST['page_cnt_photos']) {
									...
									$tpl->set('{name}', $row_owner['title']);
                  		if($row_owner['adres']) $alias = $row_owner['adres'];
				else $alias = 'public'.$row_owner['id'];
									$tpl->set('{alias}', $alias);
