...
if(isset($_SESSION['user_id']) > 0){
	...
	$user_info = $db->super_query("SELECT user_id, user_email,user_invisibility, user_group, user_friends_demands, user_pm_num, user_support, user_lastupdate, user_photo, user_msg_type, user_delet, user_ban_date, user_new_mark_photos, user_search_pref, user_status, user_last_visit, invties_pub_num FROM `".PREFIX."_users` WHERE user_id = '".$logged_user_id."'");
...
//Если есть данные о COOKIE то проверяем
	...
	if($user_info['user_password'] == $_COOKIE['password'] AND $user_info['user_hid'] == $_COOKIE['password'].md5(md5($_IP))){
		...
		$db->query("INSERT INTO `".PREFIX."_log_user` (user_id,browser,ip,date) VALUES('".$check_user['user_id']."','".$_BROWSER."','".$_IP."','".$server_time."')");
