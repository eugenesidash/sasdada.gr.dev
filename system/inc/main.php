...
if(!defined('MOZG'))
	...
	$row_users_name = $db->super_query("SELECT COUNT(*) AS cnt FROM `".PREFIX."_users_name` WHERE user_name_active = '2'");
    if($row_users_name['cnt']) $new_users_name = '<font color="red">('.$row_users_name['cnt'].')</font>';	
...
echoheader();
echoblock('payeer', 'Отчеты по счетам', 'payeer', 'payeer');
echoblock('Запрещенные сайты', 'Добавление, удаление и редактирование списка запрещенных сайтов.', 'restricted', 'restricted');
echoblock('Смена имени '.$new_users_name, 'Проверка на смену имени пользователя.', 'users_name', 'users');
echoblock('Новые подарки', 'Управление подарками на сайте, добавление, редактирование и удаление', 'new_gifts', 'gifts');
