...
//Videos Albums by 9797
var AlbumsVideos = {
        add_box: function(){
		viiBox.start();
		$.post('/index.php?go=albums_videos&act=new', function(d){
			viiBox.win('add_box', d);
		});
	},
	del: function(id){
		$.post('/index.php?go=albums_videos&act=del', {id: id}, function(d){
		Box.Info('inf', 'Информация', 'Альбом удалён!');
	...
	LoadPhoto: function(id){
		Page.Loading('start');
		$.get('/index.php?go=albums_videos&act=load_photo&id='+id, function(data){
			Box.Show('photo', 400, lang_title_load_albums, data, lang_box_canсel);
			Page.Loading('stop');
		});
	},
	ShowEdit: function(id, uid){
		$('#'+uid+', #data_doc'+id).hide();
		$('#edit_videos_tab'+id).show();
	},
	CloseEdit: function(id, uid){
		$('#'+uid+', #data_doc'+id).show();
		$('#edit_videos_tab'+id).hide();
		Page.Go(location.href);
	},
	SaveEdit: function(id, uid){
		if($('#edit_val'+id).val() != 0){
			$('#edit_videos_name'+id).text($('#edit_val'+id).val());
			$('#'+uid+', #data_doc'+id).show();
			$('#edit_videos_tab'+id).hide();
			$.post('/index.php?go=albums_videos&act=editsave', {id: id, descr: $('#edit_val'+id).val()});
		} else
			setErrorInputMsg('edit_val'+id);
			Page.Go(location.href);
	},
		editbox: function(vid){
		Box.Page('/index.php?go=albums_videos&act=edit', 'vid='+vid, 'edit_video', 415, 'Добавление видео в альбом', lang_box_canсel, lang_box_save, 'AlbumsVideos.editsave('+vid+'); return false', 137, 0, 1, 1, 0);
	...
	editsave: function(vid){
		...
		$('#box_but').hide();
		$('#box_loading').fadeIn();
		$.post('/index.php?go=albums_videos&act=addsave', {vid: vid, albums: albums}, function(d){
			Box.Info('inf', 'Информация', 'Видеозапись добавлена в альбом.');
	...
	vdel: function(vid){
		...
		$('#box_but').hide();
		$('#box_loading').fadeIn();
		$.post('/index.php?go=albums_videos&act=delsave', {vid: vid, albums: albums}, function(d){
			Box.Info('inf', 'Информация', 'Видеозапись из альбома удалёна!');
			Box.Close('edit_video');
			Page.Go(location.href);
			$('#video_object').show(); //показываем код видео, чтоб модал-окно норм появилось
		});
...
var cache = 1;
Box.Page('/index.php?go=znachok&act=view', 'user_id='+user_id, 'gifts', 679,
lang_znachok_title, lang_box_canсel, 0, 0, 450, 1, 1, 1, 0, cache);
},
send: function(gfid, fid){
$('#box_loading').show().css('margin-top', '-5px');
$.post('/index.php?go=znachok&act=send', {for_user_id: fid, gift: gfid}, function(d){if(d == 1){
...
$('body').append('<div id="newbox_miniature"><div class="miniature_box"><div class="miniature_pos" style="width: 540px;padding:15px;"><div class="miniature_title fl_l apps_box_text">История активности</div><a class="cursor_pointer fl_r" onclick="Profile.miniatureClose()">Закрыть</a><div style="border-bottom: 1px solid #DAE1E8;margin-top:20px;"></div><div class="clear"></div>'+body+'</div></div></div>');
	...
	event: function(event){
		...
		if(oi != 'privacyMenu_msg' && oi != 'privacy_lnk_msg' && oi != 'privacyMenu_wall1' && oi != 'privacy_lnk_wall1' && oi != 'privacyMenu_wall2' && oi != 'privacy_lnk_wall2' && oi != 'privacyMenu_wall3' && oi != 'privacy_lnk_wall3' && oi != 'privacyMenu_info' && oi != 'privacy_lnk_info' && oi != 'privacyMenu_gift' && oi != 'privacy_lnk_gift' && oi != 'privacyMenu_audio' && oi != 'privacy_lnk_audio' && oi != 'privacyMenu_video' && oi != 'privacy_lnk_video' && oi != 'privacyMenu_group' && oi != 'privacy_lnk_group' && oi != 'privacyMenu_notes' && oi != 'privacy_lnk_notes' && oi != 'privacyMenu_public' && oi != 'privacy_lnk_public')
			$('#privacyMenu_msg, #privacyMenu_wall1, #privacyMenu_wall2, #privacyMenu_wall3, #privacyMenu_info, #privacyMenu_gift, #privacyMenu_audio, #privacyMenu_video, #privacyMenu_group, #privacyMenu_notes, #privacyMenu_public').fadeOut(120);
	},
	savePrivacy: function(){
		...
		$.post('/index.php?go=settings&act=saveprivacy', {val_msg: val_msg, val_wall1: val_wall1, val_wall2: val_wall2, val_wall3: val_wall3, val_info: val_info, val_gift: val_gift, val_audio: val_audio, val_video: val_video, val_group: val_group, val_notes: val_notes, val_public: val_public}, function(){
			$('#ok_update').show();
...
var groups = {
	...
	saveinfo: function(id){
		...
		if(lastnews != 0){
			$('#lastnews_display').show();
			$('#e_lastnews').html(descr);
		}
		if(!adres_page)	var adres_page = 'public'+id;
		var pattern = new RegExp(/^[a-zA-Z0-9_-]+$/);
		if(pattern.test(adres_page)){
			butloading('pubInfoSave', 55, 'disabled');
			$.post('/index.php?go=groups&act=saveinfo', {id: id, title: title, descr: descr,onv: onv, lastnews: lastnews, comments: comments, pages: $('#pages').val(),adres_page: adres_page, discussion: $('#discussion').val(), web: $('#web').val()}, function(d){
				if(d == 'err_adres')
        ...
        delpublic: function(id){
                        ...
                        $.post('/index.php?go=groups&act=delpublic', {id: id}, function(d){
                                if(d == 'delete_ok')
        ...
        overspublic: function(id){
                        ...
                        $.post('/index.php?go=groups&act=overspublic', {id: id}, function(d){
                                if(d == 'delete_ok')
